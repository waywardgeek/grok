...
// runGen scaffolds a new .forge file from a Go package directory.
func runGen(pkgDir string) error {
				...
				for _, spec := range d.Specs {
					...
					default:
						typeAliases = append(typeAliases, aliasInfo{
							Name: ts.Name.Name,
							Type: goTypeToForge(typeString(ts.Type)),
						})
					}
				}
			case *goast.FuncDecl:
	...
	b.WriteString(fmt.Sprintf("// %s.forge — structural understanding of the %s package\n\n", pkgName, pkgName))
	...
	outPath := filepath.Join(absDir, pkgName+".forge")
	if _, err := os.Stat(outPath); err == nil {
		return fmt.Errorf("%s already exists — use forge update instead", outPath)
	}
...
func extractStruct(name string, st *goast.StructType, typeParams *goast.FieldList) structInfo {
	...
	if typeParams != nil {
		s.TypeParams = forgeTypeParams(typeParams)
	}
	if st.Fields != nil {
		for _, f := range st.Fields.List {
			forgeType := goTypeToForge(typeString(f.Type))
			if len(f.Names) == 0 {
				s.Fields = append(s.Fields, fieldInfo{Name: typeString(f.Type), Type: forgeType})
			} else {
				for _, n := range f.Names {
					if !isExported(n.Name) {
						continue
					}
					s.Fields = append(s.Fields, fieldInfo{Name: n.Name, Type: forgeType})
				}
			}
		}
	}
...
func extractInterface(name string, it *goast.InterfaceType, typeParams *goast.FieldList) ifaceInfo {
	...
	if typeParams != nil {
		iface.TypeParams = forgeTypeParams(typeParams)
	}
	if it.Methods != nil {
		for _, m := range it.Methods.List {
			...
			iface.Methods = append(iface.Methods, formatForgeMethod(m.Names[0].Name, ft))
		}
	}
	return iface
}

func formatForgeMethod(name string, ft *goast.FuncType) string {
	var b strings.Builder
	b.WriteString(fmt.Sprintf("func %s(", name))
	if ft.Params != nil {
		var params []string
		for _, p := range ft.Params.List {
			forgeType := goTypeToForge(typeString(p.Type))
			if len(p.Names) == 0 {
				params = append(params, forgeType)
			} else {
				for _, n := range p.Names {
					params = append(params, fmt.Sprintf("%s: %s", n.Name, forgeType))
				}
			}
		}
	...
	if ft.Results != nil && len(ft.Results.List) > 0 {
		...
		if len(results) == 1 && len(results[0].Names) == 0 {
			b.WriteString(" -> ")
			b.WriteString(goTypeToForge(typeString(results[0].Type)))
		} else {
			b.WriteString(" -> (")
			var rets []string
			for _, r := range results {
				rets = append(rets, goTypeToForge(typeString(r.Type)))
			}
...
func forgeTypeParams(fl *goast.FieldList) string {
	var parts []string
...
func goTypeToForge(goType string) string {
	switch goType {
	case "int8":
		return "i8"
	case "int16":
	...
	if strings.HasPrefix(goType, "*") {
		return goTypeToForge(goType[1:]) + "?"
	}
	if strings.HasPrefix(goType, "[]") {
		return "[" + goTypeToForge(goType[2:]) + "]"
	}
	if strings.HasPrefix(goType, "map[") {
		...
		for i, c := range goType {
			if c == '[' {
				...
				depth--
				if depth == 0 {
					return fmt.Sprintf("map[%s]%s", goTypeToForge(goType[4:i]), goTypeToForge(goType[i+1:]))
				}
			}
		}
	}
