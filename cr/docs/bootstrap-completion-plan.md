...
## Goal
Port the full Forge compiler pipeline to Forge, producing a self-hosting bootstrap compiler
that compiles .fg files → C → executable via GCC.

## Current State (as of 440f08c)
- **Done**: AST (409), Lexer (680), Parser (1412), ExprParser (1322), Desugar (1264),
  Checker (3215), LIR types (630), Lowerer (2642) = **11,574 lines Forge**
- **Compiles**: 0 GCC errors, 0 warnings, all 249+ Go tests pass
- **Missing**: C backend, monomorphizer, optimizer, main.fg, I/O builtins, `list_dir` runtime

## What Needs Porting

| Component | Go file(s) | Go lines | Go funcs | Est. Forge lines | Phase |
|-----------|-----------|----------|----------|-------------------|-------|
...
| main.fg (compile + test) | cmd/forge/main.go, shared.go | 759 | 20 | ~500 | 5 |
| **Total** | | **7,911** | **188** | **~6,080** | |

## Phase Details

### Phase 1: I/O Builtins + list_dir (~80 lines, ~30 min)
Add missing OS primitives needed by main.fg:
...
- **`mkdtemp(prefix: string) -> string`** — for `forge test` temp directories.
- **Checker**: register these in `registerBuiltins()`.
- **C backend**: add cases in `emitBuiltinCall()`.
- **Runtime**: add `forge_list_dir()`, `forge_file_exists()`, `forge_mkdtemp()` to `forge_runtime.h`.
- **Test**: write a small .fg test that calls each.

### Phase 2: Optimizer (~800 lines, ~2 hours)
Port `pkg/lir/optimize.go` → `bootstrap/optimizer/optimizer.fg`.
...
Port `pkg/lir/monomorphize.go` → `bootstrap/monomorphizer/monomorphizer.fg`.
...
Port `pkg/lir/c_backend.go` → `bootstrap/c_backend/c_backend.fg`.
...
### Phase 5: main.fg (~500 lines, ~1 hour)
Port `cmd/forge/main.go` → `bootstrap/main.fg`.
...
- `forge test`: additionally discover `test_*` functions, emit test runner, GCC compile, run.
...
1. `./forge compile bootstrap/*.fg -o /tmp/bootstrap.c`
2. `gcc -std=gnu11 -Iruntime /tmp/bootstrap.c -o /tmp/forge-bootstrap`
3. `/tmp/forge-bootstrap compile bootstrap/*.fg -o /tmp/bootstrap2.c`
4. `diff /tmp/bootstrap.c /tmp/bootstrap2.c` → identical (fixpoint!)

## Notes
- The Go→Forge ratio is roughly 0.8x (Forge is more concise due to `?`, `match`, closures).
- All modules should get `.forge` UDD files documenting invariants.
- Non-exhaustive match warnings in checker.fg are benign (4 warnings currently).
- `forge fmt` and `forge verify/update/gen` are deferred — they're tools for the Go↔Forge
